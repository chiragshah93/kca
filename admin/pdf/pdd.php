...
$pkt11.='<table border="1" align="center" width="100%">
	...
	*/
	$pkt11='<table border="1" align="center">
		<tr><th>Sr.No</th><th>Admin Id</th><th>Name</th><th>Address</th><th>Phone</th><th>User Name</th><th>Password</th><th>Agent Id</th><th>Date of Joining</th><th>Date of birth</th></tr>';
		$sn=1;
		$sql1=mysql_query("SELECT * FROM `agent`");
		while($data=mysql_fetch_assoc($sql1))
		{
		$pkt11.='<tr><td>'.$sn++.'</td><td>'.$data['adminid'].'</td><td>'.$data['name'].'</td><td>'.$data['address'].'</td><td>'.$data['phone'].'</td><td>'.$data['username'].'</td><td>'.$data['password'].'</td><td>'.$data['eid'].'</td><td>'.$data['doj'].'</td><td>'.$data['dob'].'</td></tr>';
