...
$str="select MONTHNAME(Reg_date) as m1 ,count(*)  as c1,Reg_date from tbldonor group by MONTH(Reg_date) having YEAR(Reg_date)=YEAR(CURDATE())";
...
$width = 480;  
$height = 250;  
$image = imagecreate($width, $height) or die('not create');  

// colors  
$white = imagecolorallocate($image, 0xFF, 0xFF, 0xFF);  
/*$navy = imagecolorallocate($image, 0x00, 0x00, 0x80);  */
//$navy = imagecolorallocate($image, 96, 200, 242);
$navy = imagecolorallocate($image, 0, 0, 0);
...
$padding = 15; // half of spacing between columns  
...
for ($i = 0; list($xval, $yval) = each($data); $i++) {  
    ...
    $xpos = max($xmin, $xpos);  
    $ypos = $ymax + 4; // distance from x axis  
